...
<details>
<summary><b>📡 Пошаговая инструкция для сервера (нажмите, чтобы развернуть)</b></summary>

### Установка сервера

1. Скачайте **NeoForge Server 1.21.1** с [официального сайта](https://neoforged.net/)
2. Запустите установщик и выберите **«Install server»**
3. Укажите папку для сервера и нажмите **«OK»**
4. Запустите `run.bat` (Windows) или `run.sh` (Linux)
5. Согласитесь с лицензией — откройте `eula.txt` и измените `eula=false` на `eula=true`
6. Закройте сервер

### Установка модов на сервер

1. Скачайте **все моды из таблицы выше** (кроме клиентских модов на оптимизацию, если они не предназначены для сервера)
2. Поместите `.jar` файлы в папку `mods` на сервере
3. Откройте файл `run.bat` (или `start.sh`) и измените параметр памяти:
   - Найдите `-Xmx2G` и замените на `-Xmx8G`
4. Запустите сервер

### Рекомендуемые параметры (server.properties)
